export function KindBadge({ kind }: { kind: string }) {
  ...
  return (
    <span className={`text-xs px-2 py-0.5 rounded border font-mono font-bold ${styles[kind] || 'bg-heliora-border text-heliora-text'}`}>
      {kind}
    </span>
  )
}
...
export function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`text-xs px-2 py-0.5 rounded border font-mono ${styles[status] || 'bg-heliora-border text-heliora-text'}`}>
      {status.replace(/_/g, ' ')}
    </span>
  )
}
